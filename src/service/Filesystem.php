...
class Filesystem
{
    ...
    {
        $path = str_replace(base_path() . DIRECTORY_SEPARATOR, '', rtrim(self::fsFit($path), DIRECTORY_SEPARATOR));
    ...
    public static function zip(array $files, string $fileName): bool
    {
        ...
        try {
            foreach ($files as $v) {
                $saveFile = basename($v);
                $zip->addFile($v, $saveFile);
            }
            $zip->saveAsFile($fileName);
        } catch (Throwable $e) {
            throw new Exception('Unable to package zip file');
        } finally {
            $zip->close();
        }
     ...
     * @throws Throwable
     */
    public static function zipDir(array $files, string $fileName): bool
    {
        $zip = new ZipFile();
        try {
            foreach ($files as $key => $value) {
                $zip->addDirRecursive($value, $key);
            }
            $zip->saveAsFile($fileName);
        } catch (Throwable $e) {
            throw new Exception('Unable to package zip file');
